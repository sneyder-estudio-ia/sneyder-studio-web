...
export default function CompletadosPage() {
  ...
  useEffect(() => {
    const fetchCompleted = async () => {
      setIsLoading(true);
      try {
        const qAll = query(collection(db, "orders"), where("status", "==", "completed"));
        ...
        // Sort by created_at descending (or a specific completed_at if it exists, using created_at for now)
      ...
      <header className={`fixed top-0 w-full z-50 bg-[#0c1324]/80 backdrop-blur-xl flex justify-between items-center px-4 md:px-6 h-16 shadow-[0_20px_50px_rgba(12,19,36,0.4)] transition-all duration-300 ${isMenuOpen ? "md:pl-64 lg:pl-72" : ""}`}>
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 md:gap-4 shrink-0">
            ...
            <h1 className="font-['Space_Grotesk'] tracking-tight text-lg md:text-xl font-bold text-green-400 whitespace-nowrap uppercase">
              Completados
            </h1>
          </div>
        </div>
      ...
      <main className={`pt-20 pb-28 px-4 md:px-6 max-w-7xl mx-auto space-y-8 w-full flex-grow transition-all duration-300 ${isMenuOpen ? "md:pl-64 lg:pl-72" : ""}`}>
        ...
        <section className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-outline-variant/10 pb-6">
          <div>
            ...
            <p className="text-sm text-on-surface-variant mt-1">Historial de todos los proyectos entregados satisfactoriamente.</p>
          </div>
          <div className="flex items-center gap-6 bg-green-500/5 px-6 py-4 rounded-2xl border border-green-500/10">
            <div className="flex flex-col">
              <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Completados</span>
              <span className="text-xl font-headline font-bold text-green-400">{orders.length}</span>
            </div>
            <div className="w-px h-8 bg-white/10"></div>
            <div className="flex items-center gap-2 text-[10px] text-slate-500 font-bold uppercase tracking-widest">
              <span className="material-symbols-outlined text-green-400 text-sm">inventory_2</span>
              Archivo
            </div>
          </div>
        </section>
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-surface-container-low border border-outline-variant/10 rounded-2xl pl-12 pr-4 py-4 text-sm text-white placeholder:text-slate-600 focus:outline-none focus:border-green-500/30 focus:ring-1 focus:ring-green-500/20 transition-all"
          />
          {searchTerm && (
        ...
        <section className="space-y-4">
          {isLoading ? (
            <div className="bg-surface-container-low rounded-2xl p-16 flex items-center justify-center">
              <div className="flex flex-col items-center gap-4">
                <span className="w-8 h-8 border-2 border-green-400/20 border-t-green-400 rounded-full animate-spin"></span>
                <p className="text-xs text-slate-500 uppercase tracking-widest">Cargando completados...</p>
              </div>
            </div>
          ) : filtered.length === 0 ? (
            <div className="bg-surface-container-low rounded-2xl p-16 text-center flex flex-col items-center gap-6">
              ...
              <div>
                <h3 className="text-xl font-bold mb-2">Sin Pedidos Completados</h3>
                <p className="text-slate-500 text-sm max-w-xs mx-auto">
                  {searchTerm ? "No se encontraron completados con este término." : "Aún no hay pedidos marcados como completados."}
                </p>
              </div>
            </div>
          ) : (
            <>
              <p className="text-[10px] text-slate-500 uppercase tracking-widest font-bold">
                {filtered.length} pedido{filtered.length !== 1 ? "s" : ""} completado{filtered.length !== 1 ? "s" : ""}
              </p>
              {filtered.map((order) => (
                <div key={order.id} className="bg-surface-container-low border border-green-500/10 rounded-2xl p-5 md:p-6 hover:border-green-500/25 transition-all group relative overflow-hidden">
                  {/* Green accent line */}
                  <div className="absolute top-0 left-0 w-full h-0.5 bg-gradient-to-r from-green-500/50 via-green-400/30 to-transparent"></div>
                  ...
                  <div className="flex flex-col lg:flex-row justify-between gap-5">
                    ...
                    <div className="flex flex-col gap-3 flex-1 min-w-0">
                       <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-widest px-2 py-1 rounded flex items-center gap-1.5 border bg-green-500/10 text-green-400 border-green-500/20">
                          <span className="w-1.5 h-1.5 rounded-full bg-green-400"></span>
                          Completado
                        </span>
                      </div>

                      <h3 className="text-lg font-bold font-headline group-hover:text-green-400 transition-colors truncate">
