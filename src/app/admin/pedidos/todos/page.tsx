...
export default function TodosPedidosPage() {
  ...
  const getStatusConfig = (status: string) => {
    switch (status) {
      ...
      default:
        return { label: "Pendiente", bg: "bg-yellow-500/10", text: "text-yellow-400", border: "border-yellow-500/20", dot: "bg-yellow-400" };
    }
  };
      ...
      <header className={`fixed top-0 w-full z-50 bg-[#0c1324]/80 backdrop-blur-xl h-16 shadow-[0_20px_50px_rgba(12,19,36,0.4)] transition-all duration-300 ${isMenuOpen ? "md:pl-64 lg:pl-72" : "pl-0"}`}>
        <div className="max-w-7xl mx-auto h-full px-4 md:px-6 flex justify-between items-center">
          ...
          <div className="flex items-center">
            <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="text-[#2fd9f4] hover:bg-slate-800/50 transition-all duration-300 p-2 rounded shrink-0 flex items-center justify-center">
      ...
      <main className={`flex-grow transition-all duration-300 ${isMenuOpen ? "md:pl-64 lg:pl-72" : "pl-0"}`}>
        ...
        <section className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-outline-variant/10 pb-6">
          <div>
            ...
            <p className="text-sm text-on-surface-variant mt-1">Vista completa de todos los pedidos del sistema.</p>
          </div>
          <div className="flex items-center gap-6 bg-white/5 px-6 py-4 rounded-2xl border border-white/5">
            <div className="flex flex-col">
              ...
              <span className="text-xl font-headline font-bold text-white">{orders.length}</span>
            </div>
            <div className="w-px h-8 bg-white/10"></div>
            <div className="flex flex-col">
              <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Ingresos</span>
              <span className="text-xl font-headline font-bold text-tertiary">${totalRevenue.toLocaleString()}</span>
            </div>
          </div>
        </section>
          ...
          {isLoading ? (
            ...
            <div className="bg-surface-container-low rounded-2xl p-16 text-center flex flex-col items-center gap-6">
              ...
              <div>
                ...
                <p className="text-slate-500 text-sm max-w-xs mx-auto">
                  {searchTerm ? "Intenta con otro término de búsqueda." : "Aún no hay pedidos en el sistema."}
                </p>
              </div>
            </div>
          ) : (
            <>
              ...
              {filtered.map((order) => {
                ...
                return (
                  <div key={order.id} className="bg-surface-container-low border border-outline-variant/10 rounded-2xl p-5 md:p-6 hover:border-tertiary/20 transition-all group">
                    <div className="flex flex-col lg:flex-row justify-between gap-5">
                      <div className="flex flex-col gap-3 flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <span className={`text-[10px] font-bold uppercase tracking-widest px-2 py-1 rounded flex items-center gap-1.5 border ${sc.bg} ${sc.text} ${sc.border}`}>
